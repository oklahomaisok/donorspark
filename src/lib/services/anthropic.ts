...
export async function analyzeScreenshot(
  ...
  const hasMetrics = metricsText !== 'None found';
  const metricsInstruction = hasMetrics
    ? `PRE-EXTRACTED METRICS FROM WEBSITE: ${metricsText}. IMPORTANT: Use these EXACT numbers in your metrics array - do not make up different numbers!`
    : `No metrics were pre-extracted. Look at the screenshot carefully for any impact numbers, statistics, or metrics visible on the page (e.g. "338K visits", "2,070 providers trained", "$5M raised"). Extract any real numbers you can see - do NOT make up or hallucinate numbers. If you cannot see any numbers, return an empty metrics array.`;

  const prompt = `Analyze this nonprofit website screenshot and extract brand information. LOGO URL: ${logoUrl || 'none'}. Website URL: ${url}. Organization name hint: ${orgNameHint}. ${metricsInstruction} Return a JSON object with: orgName, donorHeadline (3-6 words), heroHook (8-15 words - MUST include the full organization name for clarity), tagline, location, mission, yearFounded, sector (one of: youth-development, education, agriculture, food-bank, environment, animal-welfare, veterans, seniors, arts-culture, healthcare, housing, disaster-relief, disability-services, mental-health, refugee-immigration, lgbtq, community). SECTOR RULES: Use mental-health for crisis centers dealing with abuse, trauma, counseling, suicide prevention, child advocacy, or victim services. Use disaster-relief ONLY for natural disasters like hurricanes, floods, fires, earthquakes. Use youth-development for mentoring and child development programs, coreValues (array of 4), colors (object with primary, secondary, accent as hex), fonts (headingStyle, headingFont), need (headline and description), solution, programs (array), metrics (format as array of {value, label} objects), contactEmail, donateUrl.`;
